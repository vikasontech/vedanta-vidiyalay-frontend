/*
 *     Copyright (C) 2019  Vikas Kumar Verma
 *
 *     This program is free software: you can redistribute it and/or modify
 *     it under the terms of the GNU Affero General Public License as
 *     published by the Free Software Foundation, either version 3 of the
 *     License, or (at your option) any later version.
 *
 *     This program is distributed in the hope that it will be useful,
 *     but WITHOUT ANY WARRANTY; without even the implied warranty of
 *     MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
 *     GNU Affero General Public License for more details.
 *
 *     You should have received a copy of the GNU Affero General Public License
 *     along with this program.  If not, see <https://www.gnu.org/licenses/>.
 */
import { Component, OnInit } from '@angular/core';
import { HttpClient } from '@angular/common/http';
import { MatDialog } from '@angular/material';
import { DialogBoxUtils } from '../DialogBoxUtils';
import { catchError } from 'rxjs/operators';
import { ExceptionHandle } from '../utils/exception-handle';
import { DialogBoxComponent } from '../dialog-box/dialog-box.component';
import { UtilConfig } from '../utils/config';
import { NetworkConfig, ApiConfig } from '../utils/NetworkConfig';

@Component({
  selector: 'app-fee-due-recalculate',
  templateUrl: './fee-due-recalculate.component.html',
  styleUrls: ['./fee-due-recalculate.component.css']
})
export class FeeDueRecalculateComponent implements OnInit {
  private exception: ExceptionHandle = new ExceptionHandle();
  constructor(private http: HttpClient,  public dialog: MatDialog) { }

  ngOnInit() {
    var dialoagBoxUtils = new DialogBoxUtils();
    this.http.get(NetworkConfig.url+ApiConfig.recalculateFeeDueStatus)
    .pipe(
      catchError(this.exception.handleError(''))
    )
    .subscribe(result  => {

      var data = { message: "Fee Due status recalculate completed!", type: "Information" };

      if (result.status !== 200) {
        data = { message: result.message, type: "Error" };
        dialoagBoxUtils.openDialog(data, this.dialog);
        return;
      }

      this.dialog.open(DialogBoxComponent, {
        width: UtilConfig.dialogBoxSize,
        data: data
      });
    
    });
  }

}